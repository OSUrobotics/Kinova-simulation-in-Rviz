...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        ee_frame_camera.sendTransform(tuple(trans_ee), tuple(rot_ee), rospy.Time.now(), 'ee_frame_camera',
                                      'palm_frame_camera')
        palm_frame.sendTransform(tuple(trans), tuple(rot), rospy.Time.now(), 'palm_frame', 'aruco_endeffector')
        ee_frame_camera_flipped.sendTransform((0, 0, 0), tuple(quan), rospy.Time.now(), 'ee_frame_camera_flipped',
                                              'ee_frame_camera')
        rate.sleep()
